...
if [[ ${Q} -gt 51 ]]; then
    ...
    if [ $CODEC == "libx265" ]; then
      ffmpeg -y -i "${FILE}" -c:v ${CODEC} ${ARG_SCALE} -b:v ${Q}k -x265-params no-slow-firstpass=1:pass=1 -an -f mp4 /dev/null
      ...
      ffmpeg -y -i "${FILE}" -c:v ${CODEC} ${ARG_SCALE} -b:v ${Q}k -pass 1 -fastfirstpass 1 -an -f mp4 /dev/null
      ffmpeg -y -i "${FILE}" -c:v ${CODEC} ${ARG_SCALE} -b:v ${Q}k -pass 2 -c:a ${ARG_ACODEC} ${FILE_BASE}-${CODEC}-${Q}.${FILE_EXT} 2>&1
    else
      ffmpeg -y -i "${FILE}" -c:v ${CODEC} ${ARG_SCALE} -b:v ${Q}k -c:a ${ARG_ACODEC} ${FILE_BASE}-${CODEC}-${Q}.${FILE_EXT} 2>&1
    fi
else
    # CRF
